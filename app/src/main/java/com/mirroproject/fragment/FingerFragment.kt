 ...
 */
class FingerFragment : Fragment(), VideoViewAdapter.OnItemClickListener {
    ...
    private fun initView(view: View) {
        ...
        list_content = MirrorApplication.getInstance().getList_finger()
        ...
        rv_home_list.setLayoutManager(mLayoutManager)
    }
    ...
    private fun jujleListContent(list_content: List<VideoEntity.DataBean>) {
        if (list_content.size < 1) {
            ...
            MirrorService.getInstance().loadData(AppInfo.CODE_FINGER, mWaitDialogUtil!!, contentAdapter!!)
        }
    }
}
